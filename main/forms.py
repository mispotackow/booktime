...
class ContactForm(forms.Form):
    ...
    message = forms.CharField(label='Сообщение', max_length=600, widget=forms.Textarea)

    def send_mail(self):
        logger.info("Отправка электронной почты в службу поддержки клиентов")
        message = "From: {0}\n{1}".format(self.cleaned_data["name"],
                                          self.cleaned_data["message"],)
        send_mail("Сообщение сайта", message, "site@booktime.domain",
...
class AddressSelectionForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['shipping_addres'].queryset = queryset
