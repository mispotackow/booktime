...
# Каналы предлагают конструкции, называемые коммуникаторами.
# Коммуникаторы можно рассматривать как эквивалент тестовых клиентов,
# включенных в Django TestCase.
# Коммуникаторы, в отличие от потребителей, не имеют синхронной версии
# и асинхронной версии. Они поставляются только с асинхронным API.
# Чтобы использовать это со стандартной средой Python Unittest,
# которая работает синхронно, нам нужно будет использовать некоторый
# низкоуровневый asyncio API.
...
#                 last_name="Smith",)
#             order = factories.OrderFactory(user=user)
#             cs_user = factories.UserFactory(email="customerservice@booktime.domain",
#                                             first_name="Adam",
#                                             last_name="Ford",
#                                             is_staff=True,)
...
#             self.assertEquals(await communicator.receive_json_from(), {"type": "chat_join", "username": "John Smith"},)
#             self.assertEquals(await communicator.receive_json_from(), {"type": "chat_join", "username": "Adam Ford"},)
#             self.assertEquals(await communicator.receive_json_from(),{"type": "chat_message",
#                                                                       "username": "John Smith",
#                                                                       "message": "hello customer service",},)
#             self.assertEquals(await communicator.receive_json_from(), {"type": "chat_message",
#                                                                        "username": "Adam Ford",
...
#     def test_chat_blocks_unauthorized_users(self):
#         def init_db():
#             user = factories.UserFactory(email='john@bestemails.com',
#                                          first_name='John',
#                                          last_name='Smith',)
#             order = factories.OrderFactory()
#
...
#         async def test_body():
#             user, order = await database_sync_to_async(init_db)()
#             communicator = WebsocketCommunicator(consumers.ChatConsumer,
#                                                  '/ws/customer-service/%d/' % order.id,)
#             communicator.scope['user'] = user
#             communicator.scope['url_route'] = {'kwargs': {'order_id': order.id}}
#             connected, _ = await communicator.connect()
...
#             await communicator.disconnect()
#
#             communicator = HttpCommunicator(consumers.ChatNotifyConsumer,
#                                             'GET',
#                                             '/customer-service/notify/',)
#             communicator.scope['user'] = notify_user
#             communicator.scope['query_string'] = 'nopoll'
...
#             payload = response['body'][6:]
#             data = json.loads(payload.decode('utf8'))
#             self.assertEquals(data, [{'link': '/customer-service/%d/' % order.id,
#                                       'text': '%d (user@site.com)' % order.id,}],
